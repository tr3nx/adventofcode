...
-- part one
parseInputPath :: String -> (Int, Int)
parseInputPath ('U':amount) = (0, read amount)
parseInputPath ('R':amount) = (1, read amount)
parseInputPath ('D':amount) = (2, read amount)
parseInputPath ('L':amount) = (3, read amount)

xyFromPath :: (Int, Int) -> (Int, Int) -> (Int, Int)
xyFromPath (x, y) (0, a) = (x, y + a) -- UP
xyFromPath (x, y) (1, a) = (x + a, y) -- RIGHT
xyFromPath (x, y) (2, a) = (x, y - a) -- DOWN
xyFromPath (x, y) (3, a) = (x - a, y) -- LEFT

xyFromPaths :: [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)]
xyFromPaths keep []    = keep
xyFromPaths keep (x:xs) = xyFromPaths (keep ++ [xyFromPath pos x]) xs
 where
  prev = last keep
  pos = (fst prev, snd prev)

xysBetween :: (Int, Int) -> (Int, Int) -> [(Int, Int)]
xysBetween (a, b) (c, d) = if a == c
  then [(a, y) | y <- ny]
  else [(x, b) | x <- nx]
 where
  nx = if a <= c then [a..c] else reverse [c..a]
  ny = if b <= d then [b..d] else reverse [d..b]

xyBetweens :: [(Int, Int)] -> [(Int, Int)] -> [(Int, Int)]
xyBetweens keep []     = keep
xyBetweens keep (x:xs) = keep ++ xyBetweens (xysBetween (last keep) x) xs

-- part two
doesNotMatch :: (Int, Int) -> (Int, Int) -> Bool
doesNotMatch (a, b) (c, d) = if (a == c) == False then True else if (b == d) == False then True else False

solve :: [String] -> [String] -> Int
solve one two = do
 let ones = tail $ nub $ xyBetweens [(0, 0)] $ xyFromPaths [(0, 0)] $ map parseInputPath one
 let twos = tail $ nub $ xyBetweens [(0, 0)] $ xyFromPaths [(0, 0)] $ map parseInputPath two
 let crossings = filter (`elem` ones) twos
 let stepsOne = map (\x -> length $ takeWhile (\y -> doesNotMatch x y) ones) crossings
 let stepsTwo = map (\x -> length $ takeWhile (\y -> doesNotMatch x y) twos) crossings
 head $ sort $ zipWith (+) (map abs stepsOne) (map abs stepsTwo) + 3 -- plus three...

main = do
 let inputsOne = ["R992","U221","L822","U805","R667","D397","L969","U433","R918","D517","L494","U909","L224","D738","R247","D312","L803","D656","L571","D968","L392","D332","L581","U487","R522","D780","L74","D561","L246","U380","L125","U11","R735","D761","R482","D208","R985","D991","L352","U140","L586","D492","L777","U96","R682","D969","R775","U279","R671","D423","R838","U907","L486","D702","L432","D625","R463","U559","R12","D531","R510","D347","R147","U949","R175","U160","L975","D627","L537","D343","L406","D237","R953","U725","L996","D740","L703","D996","R157","U356","R247","D541","L592","D345","R580","U656","R50","D423","L158","U502","L86","U729","L720","D464","R901","D739","L20","U21","R497","D14","L580","U610","L114","D858","R853","U550","L354","D433","L507","U144","R9","U422","R674","U604","R107","D999","L420","U675","R538","D491","R84","D158","R303","D450","L616","U938","L162","U102","L160","U275","L281","D164","L254","U103","R60","D707","R655","U128","L907","U225","L292","U919","R517","D276","R308","D113","L455","U584","R899","U321","L417","U449","L780","U387","L579","U224","L192","D325","L626","U145","R178","D162","L18","D469","R169","U694","R162","D806","L10","U979","L944","D304","R719","D253","L343","D711","R429","D933","R445","D772","R230","D407","R335","U883","L900","D377","R413","D44","R805","D378","R421","D860","L597","U63","L583","D561","R235","D502","L37","U29","L381","U803","R588","D972","R678","D223","L440","U835","R88","D16","R529","D867","R742","U25","R353","D952","R31","D202","R737","D744","R765","U154","L969","U851","L22","U165","L12","D457","R635","U829","L996","D871","L397","U995","R215","D505","R93","U12","R183","D920","L442","D393","L919","D803","R22","D806","R776","U558","R263","D222","R111","D530","L908","D640","R351","D172","R315","U731","R25","U718","L172","D145","L606","U803","R837","U310","L607","D523","R271","U927","R3","U518","R754","D322","L924","D256","L997","U153","L904","D745","L475","U346","L979","D658","R208","U924","L484","U961","R94","D283","L79","U927","R122","D513","L806","D480","L971","U340","R328","D427","L494"]
 ...
 putStrLn $ "Answer: " ++ show (solve inputsOne inputsTwo)
