...
solve one two = do
 let pathOne = map parseInputPath one
 let pathTwo = map parseInputPath two
 let xysOne  = xyFromPaths [(0, 0)] pathOne
 let xysTwo  = xyFromPaths [(0, 0)] pathTwo
 let lastOne = tail $ nub $ xyBetweens  [(0, 0)] xysOne
 let lastTwo = tail $ nub $ xyBetweens [(0, 0)] xysTwo
 head $ sort $ map (\x -> abs (fst x) + abs (snd x)) $ filter (`elem` lastTwo) lastOne

main = do
 -- 6
 -- let inputsOne = ["R8","U5","L5","D3"]
 -- let inputsTwo = ["U7","R6","D4","L4"]

 -- 159
 -- let inputsOne = ["R75","D30","R83","U83","L12","D49","R71","U7","L72"]
 -- let inputsTwo = ["U62","R66","U55","R34","D71","R55","D58","R83"]

 -- 135
 -- let inputsOne = ["R98","U47","R26","D63","R33","U87","L62","D20","R33","U53","R51"]
 -- let inputsTwo = ["U98","R91","D20","R16","D67","R40","U7","R15","U6","R7"]
