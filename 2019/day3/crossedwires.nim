...
template last[T](xs: openarray[T]): T =
  xs[xs.len - 1]

proc makePos(dir, steps, x, y: int): Pos =
  if   dir == 0: result = (x, y + steps)
  elif dir == 1: result = (x + steps, y)
  elif dir == 2: result = (x, y - steps)
  elif dir == 3: result = (x - steps, y)
...
proc hasPoint(ps: seq[Pos], pos: Pos): bool =
  ps.filter(p => pos.x == p.x and pos.y == p.y).len > 0

proc makePoints(ps: seq[string]): seq[Pos] =
  result.add((0,0))
  for (dir, steps) in ps.mapIt(parsePath(it)):
    let prev = result.last
    result.add(makePos(dir, steps, prev.x, prev.y))

proc connectPoints(ps: seq[Pos]): seq[Pos] =
  for (i, pos) in ps.pairs:
    var point = ps[0]
    if i > 1:
      point = ps.last
    result.add(makeBetweens(point, pos))
  result.deduplicate
...
let firstLine = connectPoints(makePoints(first))
let secondLine = connectPoints(makePoints(second))

echo firstLine.filterIt(secondLine.contains(it)).pop.calcDistance
