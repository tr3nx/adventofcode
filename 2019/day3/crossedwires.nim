...
type
  Path = tuple[dir, steps: int]
  Pos = tuple[x, y: int]

proc toDirection(c: char): int =
  if   c == 'R': result = 1
  elif c == 'D': result = 2
  elif c == 'L': result = 3
  else:          result = 0 # 'U'

proc parsePath(p: string): Path =
  (toDirection(p[0]), parseInt(p[1..^1]))

proc makePos(x, y, d, a: int): Pos =
  if   d == 0: result = (x, y + a) # UP
  elif d == 1: result = (x + a, y) # RIGHT
  elif d == 2: result = (x, y - a) # DOWN
  elif d == 3: result = (x - a, y) # LEFT

proc makeBetweens(start, stop: Pos): seq[Pos] =
  result = @[]
  let
    a = start.x
    b = start.y
